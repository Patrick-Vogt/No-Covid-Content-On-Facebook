...
function setSwitch() {
	...
	chrome.storage.sync.get(['coronaOn'], function (item) {
		...
		if (coronaOn) {
			...
			document.getElementById("subtext").innerHTML = "Covid-Related posts <b>won't</b> appear on your Facebook feed";
		} else {
			chrome.browserAction.setIcon({ path: "images/iconoff128.png" });
			document.getElementById("subtext").innerHTML = "Covid-Related posts <b>will</b> appear on your Facebook feed";
		}
	});
}
...
function updateOptions() {
	...
	chrome.storage.sync.set({ 'coronaOn': select.checked }, function () {
		if (select.checked) {
			chrome.browserAction.setIcon({ path: "images/icon128.png" });
			document.getElementById("subtext").innerHTML = "Covid-Related posts <b>won't</b> appear on your Facebook feed <b>*reload facebook*</b>";
			document.getElementById("subtext2").style.display = "block";
			document.getElementById("subtext3").style.display = "none";
		} else {
			chrome.browserAction.setIcon({ path: "images/iconoff128.png" });
			document.getElementById("subtext").innerHTML = "Covid-Related posts <b>will</b> appear on your Facebook feed <b>*reload facebook*</b>";
